...
检查返回：`status` 应为 `SUCCESS`，`chunkCount` > 0。

**问答**

```powershell
$chatBody = '{"question":"RagChunk 一期离线有几步？"}'
Invoke-RestMethod -Method Post `
...
2. 打开任一入口：
   - **Swagger UI（推荐）**：http://localhost:8080/swagger-ui/index.html
   - **兼容入口**：http://localhost:8080/doc.html（重定向到 Swagger UI）
3. 右上角分组选 **RagChunk 一期 API**，按顺序调试：
   - **知识库** → `POST /api/v1/knowledge-bases` → 复制返回的 `id`
   - **文档** → `POST .../documents` → 填 `kbId`，上传文件，`smartChunk` 可选
   - **问答** → `POST .../chat` → 填 `kbId` 与 `question`

依赖：`springdoc-openapi-starter-webmvc-ui` **3.0.3**（与 Spring Boot 4 匹配）。
