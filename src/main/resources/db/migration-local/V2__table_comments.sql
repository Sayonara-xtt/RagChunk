-- 表与字段注释（array 模式，与 db/migration/V2 一致）
...
COMMENT ON TABLE chunk IS '切片与向量：检索主表（embedding 为 real[]）';
...
COMMENT ON COLUMN chunk.embedding IS '浮点数组向量，维度与 knowledge_base.embedding_dimensions 一致';
